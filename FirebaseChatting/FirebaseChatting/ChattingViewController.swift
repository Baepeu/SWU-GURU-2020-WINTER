...
class ChattingViewController:MessagesViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        db.collection("chattings").document(chat_room_id!).collection("chat").order(by: "date").addSnapshotListener { (snapshop, error) in
            if let error = error {
                return
            }
            if let snapshop = snapshop {
                snapshop.documentChanges.forEach { change in
